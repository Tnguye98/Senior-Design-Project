...
# import os
# Used to actually import and export files. Could be used to take in pictures externally and return new files?
...
# Not importing backend as K will allow tf to run a bit further, but then run into errors K dependant functions
# keras 2.6 no longer uses this method, use tf.keras.backend.<function> to replace previous dependencies

# from tensorflow.keras import backend as K
# Go to tensorflow 2.4 and try if problems persist with code
...
# IMAGE_DIMENSIONS should be edited to accommodate differently sized images.
...
# Used to implement epsilon, needed for the variational part
def sampling(args):
    z_mean, z_log_var = args

    # epsilon = k.random_normal(shape=(K.shape(z_mean)[0], LATENT_DIM),mean=0., stddev=1.0)
    # K.random_normal does not seem to be the format for keras 2.6 anymore, so changed to tf.keras and see what happens
    pre_epsilon = tf.keras.initializers.RandomNormal(mean=0., stddev=1.0)
    epsilon = pre_epsilon(shape=(tf.shape(z_mean)[0], LATENT_DIM))
    # TypeError called: __init__() got an unexpected keyword argument 'shape'
    # Created shape for Random Normal

    # return z_mean + k.exp(z_log_var) * epsilon
    # keras no longer has k.exp() as a function in 2.6, so we will use the tf.math library to replace it
...
# Old method of calling layers
x = tf.keras.layers.Conv2D(3, kernel_size=(4, 4), padding='same', activation='relu', name='RGB_Layer')(encoder_input)
...
# Old method of calling layers
x = tf.keras.layers.Conv2D(32, kernel_size=(4, 4), padding='same', activation='relu', strides=(4, 4),name='Conv_Layer_1')(x)
x = tf.keras.layers.MaxPooling2D((2, 2), padding='same', name='Pooling_Layer_1')(x)
...
x = tf.keras.layers.Conv2D(64, kernel_size=(4, 4), padding='same', activation='relu', strides=1, name='Conv_Layer_2')(x)
x = tf.keras.layers.MaxPooling2D((2, 2), padding='same', name='Pooling_Layer_2')(x)
...
# Old method of calling layers
x = tf.keras.layers.Conv2D(64, kernel_size=1, padding='same', activation='relu', strides=1, name='Conv_Layer_3')(x)
...
# Old method of calling layers
flat_layer = tf.keras.layers.Flatten(name='Flatten_Layer')(x)
hidden_layer = tf.keras.layers.Dense(HIDDEN_LAYER_DIM, name='Hidden_Layer')(flat_layer)
...
# Old method of calling layers
z_mean = tf.keras.layers.Dense(LATENT_DIM, name='Z_MEAN')(hidden_layer)
z_log_var = tf.keras.layers.Dense(LATENT_DIM, name='Z_LOG_VAR')(hidden_layer)

# Insert epsilon via sampling layer
# encoder_output = layers.Lambda(sampling, output_shape=(LATENT_DIM,), name='Latent_Space')([z_mean, z_log_var])
# This calls to the tf 1.X method of calling Keras' layers.Lambda
encoder_output = tf.keras.layers.Lambda(sampling, output_shape=(LATENT_DIM,), name='Latent_Space')([z_mean, z_log_var])


# Complete encoder model
# encoder = keras.Model(encoder_input, encoder_output, name="encoder")
# This calls to the tf 1.X method of calling keras.Model
encoder = tf.keras.Model(inputs=encoder_input, outputs=encoder_output, name="encoder")
# Running into ValueError: Output tensors of a Function model must be the output of a tensorFlow 'Layer'
# See line 201, as output is a Lambda layer
...
# Old method of calling layers
x = tf.keras.layers.Dense(HIDDEN_LAYER_DIM, name='Hidden_Layer')(decoder_input)
...
# Old method of calling layers
x = tf.keras.layers.Dense(64 * 32 * 32, name='Upscale_Layer')(x)
...
# Old method of calling layers
x = tf.keras.layers.Conv2DTranspose(64, kernel_size=1, padding='same', strides=1, activation='relu', name='TP_Layer_3')(x)
...
# Old method of calling layers
x = tf.keras.layers.UpSampling2D((2, 2), name="UpSample_Layer_2")(x)
x = tf.keras.layers.Conv2DTranspose(64, kernel_size=(4, 4), padding='same', strides=1, activation='relu',name='TP_Layer_2')(x)
...
# Old method of calling layers
x = tf.keras.layers.UpSampling2D((2, 2), name="UpSample_Layer_1")(x)
x = tf.keras.layers.Conv2DTranspose(32, kernel_size=(4, 4), padding='same', strides=(4, 4), activation='relu',name='TP_Layer_1')(x)
...
# Old method of calling layers
decoder_output = tf.keras.layers.Conv2D(3, kernel_size=(4, 4), padding='same', activation='sigmoid',name='Transpose_RGB_Layer')(x)
...
# Old way of using Model
vae = tf.keras.Model(inputs=encoder_input, outputs=output, name="vae")
vae.summary()
...
# Relies on old 'backend as K' that does not work in Keras 2.6; need to replace
base_truth = tf.reshape(encoder_input, [-1])

# predicted_truth = K.flatten(output)
# Relies on old 'backend as K' that does not work in Keras 2.6; need to replace
predicted_truth = tf.reshape(output, [-1])
...
for epoch in range(start_at_epoch, max_epochs):
    ...
    # NOTE: This is bottleneck by I/O speed. Image size may also play a role.
    # 	If RAM allows, consider loading all data at once?
    # IDEA: Implement these asynchronously for less bottleneck?
    ...
    if epoch in epoch_plot_step:
        ...
        # Brab a sample and save it to validation plot data
